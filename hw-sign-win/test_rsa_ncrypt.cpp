...
private:
    ...
    NCRYPT_KEY_HANDLE hHwRsaKey_ = NULL;
    EC_KEY* accelEcdhKey_ = nullptr;
    std::vector<uint8_t> sharedSecret_;
    std::string keyType_ = "rsa-2048-pss";
...
public:
    HybridCryptoHelper() {
        ...
        // Initialize NCrypt for hardware RSA key
        ...
        // Generate RSA 2048-bit key for hardware key
        ...
        if (FAILED(status)) {
            NCryptFreeObject(hHwRsaKey_);
            throw std::runtime_error("Failed to set RSA key length");
        }

        // Finalize the key
        status = NCryptFinalizeKey(hHwRsaKey_, 0);
        if (FAILED(status)) {
            NCryptFreeObject(hHwRsaKey_);
        ...
        std::cout << "Generated RSA 2048-bit hardware key (NCrypt) and ECDH P-256 acceleration key (OpenSSL)" <<
            std::endl;
    }

    ~HybridCryptoHelper() {
        if (hHwRsaKey_) {
            NCryptDeleteKey(hHwRsaKey_, 0);
            NCryptFreeObject(hHwRsaKey_);
        }
        if (hProvider_) {
...
public:
    ...
    void runFullTest() {
        ...
        std::cout << "RSA-2048 Hardware Key + ECDH Accel Key" << std::endl;
        std::cout << "Using NCrypt (Hardware) + OpenSSL (Everything Else)" << std::endl;
        std::cout << "==========================================" << std::endl;

        std::string username = "testuser_rsa_hybrid_" + std::to_string(std::time(nullptr));
        std::string password = "testpass123";

        // Test 1: Register
        bool registerSuccess = testRegister(username, password);

        // Test 2: Login with RSA hardware key
        ...
        std::cout << "Login (RSA HW key):     " << (loginSuccess ? "✓ PASS" : "✗ FAIL") << std::endl;
        ...
        if (allPassed) {
            ...
            std::cout << "✓ RSA-2048 hardware key authentication works (NCrypt)" << std::endl;
            std::cout << "✓ ECDH acceleration key exchange works (OpenSSL)" << std::endl;
...
int main() {
    try {
        ...
        std::cout << "Testing RSA-2048 hardware key with ECDH acceleration key" << std::endl;
        std::cout << "Using hybrid approach: NCrypt for hardware RSA, OpenSSL for everything else" << std::endl;
