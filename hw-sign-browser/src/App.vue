...
import { register, login, isAuthenticated as checkAuthService } from './services/authService';
...
async function handleRegister() {
  try {
    ...
    if (response.success) {
      ...
      message.value = 'Registration failed: ' + response.message;
    }
  } catch (error) {
    message.value = 'Error during registration: ' + error;
  }
}
    ...
    <div class="w-full max-w-lg bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8 border border-gray-200 dark:border-gray-700">
      ...
      <div v-if="!isAuthenticated">
        ...
        <div class="flex justify-between">
          <button @click="handleRegister" class="bg-green-600 text-white py-3 px-4 rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900">Register</button>
          <button @click="handleLogin" class="bg-indigo-600 text-white py-3 px-4 rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900">Login</button>
        </div>
